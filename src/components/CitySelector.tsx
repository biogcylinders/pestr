...
export function CitySelector({ currentServiceSlug, currentCitySlug }: CitySelectorProps) {
  return (
          ...
          {cities.map((c) => {
            ...
            return (
              <Link
                key={c.slug}
                to="/$service/$city"
                params={{ service: currentServiceSlug, city: c.slug }}
                className={`relative inline-flex items-center justify-center rounded-lg px-3 py-1.5 text-xs font-medium transition-all duration-200 ${
                  isActive
